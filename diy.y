...
void bFunction(char *name, int enter, Node *init, int type);
void bDeclare(char *name, Node *vc, Node *init, int cnst);
void externs();
...
%token LOCAL POSINC POSDEC PTR CALL START PARAM NIL DECLS
%%
...
file	:
	...
	| file public tipo ID { enter($2, $3->value.i, $4); } finit { if($3->value.i != 3) function($2, $3, $4, $6, 4 + enterVal); else function($2, $3, $4, $6, 8 + enterVal);}
	| file public VOID ID { enter($2, 4, $4); } finit { function($2, intNode(VOID, 4), $4, $6, enterVal); }
	;
...
finit   : '(' { pos = 8; } params ')' { if (voidFlag) pos = 0; else pos = $<i>-3 != 3 ? -4 : -8; } blocop { $$ = binNode('(', $6, $3); }
	| '(' ')' { if (voidFlag) pos = 0; else pos = $<i>-3 != 3 ? -4 : -8; } blocop   { $$ = binNode('(', $4, 0); }
	;
...
void enter(int pub, int typ, char *name) {
	...
	IDpush();
	if (typ != 4 && typ != 3) { IDnew(typ, name, -4); voidFlag = 0; }
	else if (typ == 3) { IDnew(typ, name, -8); voidFlag = 0; }
	else voidFlag = 1;
}
...
void function(int pub, Node *type, char *name, Node *body, int enter)
{
	...
	if (bloco != 0) { /* not a forward declaration */
		...
		bFunction(name, enter, bloco, type->value.i);
	}
